...
stutest (Avg1, Avg2, Var1, Var2, n1, n2)
    double Avg1, Avg2, Var1, Var2;
    int n1, n2;
{
	...
	if (Fprob<0.0) printf("Cannot perform F-test\n");
	...
	if (tprob<0.0) printf("Cannot perform t-test\n");
	/* output to stdout */
		...
		int *n1, int *n2)
{
    ...
    if (NULL==fgets(Line, 130, stdin)) return(0);	/* possibly EOF */
    if (Line[0]=='#') return(-2);   /* comment */
    if (6>sscanf(Line, "%lf %lf %d %lf %lf %d", 
	    Avg1, Sd1, n1, Avg2, Sd2, n2))
	return(-1);	/* cannot parse line */
    else return(1);	/* OK */
}
/* END of get_avgsd */
	...
	double Var2, double Fprob, double tprob)
{
    ...
    printf("Avg: %.2e %s %.2e Tprob=%.2e ", 
	    Avg1, (tprob<SIGNIF)? ((Avg1<Avg2)? "<": ">"):"=", Avg2, tprob);
    if (tprob<SIGNIF) putchar('*');
    if (tprob<VERY_SIGNIF) putchar('*');    /* second asterisk */
    putchar('\n');
    ...
    printf("StD: %.2e %s %.2e Fprob=%.2e ", 
	    Sd1, (Fprob<SIGNIF)? ((Sd1<Sd2)? "<": ">"):"=", Sd2, Fprob);
    if (Fprob<SIGNIF) putchar('*');
    if (Fprob<VERY_SIGNIF) putchar('*');    /* second asterisk */
    putchar('\n');
...
static double incompl_beta(double a, double b, double x)
{
    ...
    bt=ln_gamma(a+b)-ln_gamma(a)-ln_gamma(b)+a*log(x)+b*log(1.0-x);
    bt=exp(bt);
...
static double ln_gamma(double xx)
{
    ...
    static const double Coeff[6]={76.18009172947146, -86.50532032941677, 24.01409824083091, 
...
static double beta_cf(double a, double b, double x)
{
	...
	if ((fabs(az-aold)) < (EPS*abs(az))) return(az);
    }
