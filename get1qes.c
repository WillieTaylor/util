/*
cc get1qes.c -o get1qes util/wt/util.o util/aa/pdbprot.o -lm
*/
#include <stdlib.h>
#include <alloca.h>
	...
	{ int	nres = prot->Chains[j].Aano,
	  ...
	  float asum, bsum, apct, bpct;
		...
		{ int	difs = 0;				/* unless it is identical */
			...
			if (difs) {
				for (i=0; i<nres; i++) {
					printf("%c", prot->Chains[j].Atoms[i].Aa);
				}
				continue;
			}
		}
		...
		printf(">%s%c\n",code,chid);
		printf("%s-reversed %c",prot->Pdbcode, chid);
