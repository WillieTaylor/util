...
int i, key[4], ley[4];
	...
	{ int ke1 = key[1], ke2 = key[2],
	      le1 = ley[1], le2 = ley[2];
	  Vec tmp1, tmp2, tmp;
for(i=0; i<4; i++) printf("%d", key[i]); NL
for(i=0; i<4; i++) printf("%d", ley[i]); NL
...
Pv(y) NL Pv(h) NL Pv(tmp) NL Pv(box[3]) NL
			} else {
				vcopy(x,&tmp); vmul(&tmp,t[ke2]); vadd(g,tmp,box+3);
Pv(x) NL Pv(g) NL Pv(tmp) NL Pv(box[3]) NL
			}
		} else { /* staggered */
			vcopy(tmp2,box+3);
			if (le2==0) {
				vcopy(y,&tmp); vmul(&tmp,t[ke2]); vadd(h,tmp,box+1);
Pv(y) NL Pv(h) NL Pv(tmp) NL Pv(box[1]) NL
			} else {
				vcopy(x,&tmp); vmul(&tmp,t[ke2]); vadd(g,tmp,box+1);
Pv(x) NL Pv(g) NL Pv(tmp) NL Pv(box[1]) NL
			}
		}
Pi(ke1) Pr(t[ke1]) NL
		if (le1==0) {
			vcopy(y,&tmp); vmul(&tmp,t[ke1]); vadd(h,tmp,box+2);
Pv(y) NL Pv(h) NL Pv(tmp) NL Pv(box[2]) NL
		} else {
			vcopy(x,&tmp); vmul(&tmp,t[ke1]); vadd(g,tmp,box+2);
Pv(x) NL Pv(g) NL Pv(tmp) NL Pv(box[2]) NL
		}
	}
	return lap;
}
