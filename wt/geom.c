...
void MtoM ( Mat *M, Mat *T ) {
        T->A.x = M->A.x; T->B.x = M->B.x; T->C.x = M->C.x;
        T->A.y = M->A.y; T->B.y = M->B.y; T->C.y = M->C.y;
        T->A.z = M->A.z; T->B.z = M->B.z; T->C.z = M->C.z;
}
...
void MmulM ( Mat *P, Mat *Q, Mat *R ) {
	...
	R->A.x = P->A.x*Q->A.x + P->B.x*Q->A.y + P->C.x*Q->A.z;
        R->B.x = P->A.x*Q->B.x + P->B.x*Q->B.y + P->C.x*Q->B.z;
        R->C.x = P->A.x*Q->C.x + P->B.x*Q->C.y + P->C.x*Q->C.z;

	R->A.y = P->A.y*Q->A.x + P->B.y*Q->A.y + P->C.y*Q->A.z;
        R->B.y = P->A.y*Q->B.x + P->B.y*Q->B.y + P->C.y*Q->B.z;
        R->C.y = P->A.y*Q->C.x + P->B.y*Q->C.y + P->C.y*Q->C.z;

	R->A.z = P->A.z*Q->A.x + P->B.z*Q->A.y + P->C.z*Q->A.z;
        R->B.z = P->A.z*Q->B.x + P->B.z*Q->B.y + P->C.z*Q->B.z;
        R->C.z = P->A.z*Q->C.x + P->B.z*Q->C.y + P->C.z*Q->C.z;
}
...
Vec	A,B,C, a,b,c;
float	sr, cr;
	vcopy(R->A,&A); vcopy(R->B,&B); vcopy(R->C,&C);
	a.x = a.y = a.z = 0.0;
	b.x = b.y = b.z = 0.0;
	c.x = c.y = c.z = 0.0;
	cr = cos(r); sr = sin(r);
...
/* min dist between line segments a-b and c-d */
Vec x, y, z;
Mat M[1], W[1];
float det, dmin;
Vec e,f;
...
void dot2line ( Vec a, Vec b, Vec c, Vec *g ) {
// finds the image of c on a-b line in g
Vec p, q;
float d;
	vsub(b,a,&p);
	vsub(c,a,&q);
...
int dot1pair ( Vec a, Vec b, Vec c, Vec *g ) {
/* returns 1 if c lies over the line a-b and image of c on a-b in g */
Vec p, q;
float d;
	vsub(b,a,&p);
	vsub(c,a,&q);
...
/* returns the (unsigned) angle at b (in rad)*/
float d;
Vec x, y;
	vsub(a,b,&x);
	if (vsqr(x) < NOISE) return 0.0; // b lies on a
	vnorm(&x);
	vsub(c,b,&y);
	if (vsqr(y) < NOISE) return 0.0; // b lies on c
	vnorm(&y);
	d = vdot(x,y);
...
float vol, cos, sin, tor;
	if (vddif(a,d) < NOISE) return PI;
	tor = 2.0*(drand48()-0.5)*PI; // default random
	vsub(a,b,&x);
	if (vsqr(x) < NOISE) return tor; // b lies on a
	vsub(b,c,&y);
	if (vsqr(y) < NOISE) return tor; // b lies on c
	vsub(c,d,&z);
	if (vsqr(z) < NOISE) return tor; // d lies on c
	vprod(x,y,&p);
	if (vsqr(p) < NOISE) return tor; // x,y colinear
	vnorm(&p);
	vprod(y,z,&q);
	if (vsqr(q) < NOISE) return tor; // y,z colinear
	vnorm(&q);
	cos = vdot(p,q);
	vprod(p,q,&r);
	vol = vtri(p,q,y);
...
int fsort4min ( float a, float b, float c, float d ) {
...
int fsort4max ( float a, float b, float c, float d ) {
	if ( a>b && a>c && a>d ) return 1;
	if ( b>a && b>c && b>d ) return 2;
	if ( c>a && c>b && c>d ) return 3;
	if ( d>a && d>b && d>c ) return 4;
...
int isort4min ( int a, int b, int c, int d ) {
	if ( a<b && a<c && a<d ) return 1;
	if ( b<a && b<c && b<d ) return 2;
	if ( c<a && c<b && c<d ) return 3;
	if ( d<a && d<b && d<c ) return 4;
...
int isort4max ( int a, int b, int c, int d ) {
	if ( a>b && a>c && a>d ) return 1;
	if ( b>a && b>c && b>d ) return 2;
	if ( c>a && c>b && c>d ) return 3;
	if ( d>a && d>b && d>c ) return 4;
