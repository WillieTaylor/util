...
cc -O multas.c -o multas seqsort.o util/wt/util.o util/wt/sort.o -lm  
	...
	{ int	*pep_order;
	  int	i;
		...
		{ int	aspan, bspan, minp, hold, diag;
			fscanf(run_data,"%d%d%d%d%d",
				&aspan,&bspan,&minp,&hold,&diag);
			if (aspan < 0) aspan = nblocks/(i+2);
			if (bspan < 0) bspan = last_span;
			npairs = count
				(nblocks,aspan,bspan,pep_order,minp,hold,diag);
			last_span = aspan;
			pre_order(pep_order);
			pep_order[0]++;
		}
...
{	int	i, j, k;
	...
	char	*c;
	FILE	*seq, *fas, *afa;
	seq = fopen("final.ali","w");
	fas = fopen("final.fas","w");
	afa = fopen("final.afa","w");
	Pi(nblocks) NL
	for ( i = 0; i < nblocks; i++ ) 
	{ 	Blocks	*b = block[i];
		...
		int	seed = -1;;
		if (!b_memb) continue;
                for ( j=0; j<b_memb; j++ ) { Seqs  *s = b->seq[j];
		...
		{ Seqs	*s = b->seq[j];
		  ...
		  float resol, bvalue;
			fprintf(seq,">P1;%s\n", s->code);
			fprintf(seq,"sequence = %s = %s\n", s->code+5, s->title);
			fprintf(fas,">%s %s\n", s->code,s->title);
			c = strchr(s->title,','); if (c) *c = (char)0;
			fprintf(afa,">%s %s\n", s->code,s->title);
			for ( k = 1; k <= b->length; k++ ) { char	res;
				if (k > s->length) res = ' ';
					else	   res = s->residue[k];
				res = s->residue[k];
				if (res<'A' || res>'Z') res = '-';
				fprintf(seq,"%c",res); 
				fprintf(fas,"%c",res); 
				if (seq0->residue[k]<'A') continue;
				if (seq0->residue[k]>'Z') continue;
				fprintf(afa,"%c",res); 
			}
			fprintf(seq,"*\n");
			fprintf(fas,"\n");
...
{	Blocks	*b;
	...
	if ( seq.len > maxs ) {
		...
		printf("*NB* sequence too long\n");
		return 0;
	}
	if ( seq.len < mins ) {
		printf("%d %s %d %s\n", nseqs,seq.name,seq.len,seq.doc);
		printf("*NB* sequence too short\n");
		return 0;
	}
	...
	if (delayed==1) {
		...
		b->seq = (Seqs**)malloc(sizeof(Seqs*)); TEST(b->seq)
	...
	if (key) nkey++;
	if (delayed==1)
		copy_seq(key, s->code, seq.seq, seq.seq, seq.len);
	else	s->odd = (int)
		copy_seq(key, s->code, seq.seq, s->residue,  seq.len);
	// allocate profile
	b->prf = (unsigned char**)malloc(sizeof(unsigned char*)*NACID); TEST(b->prf)
	for ( i=0; i<NACID; i++) {
		b->prf[i] = (unsigned char*)malloc(sizeof(unsigned char)*(b->length+1));
		for (j=0; j<=b->length; j++) b->prf[i][j] = 0;
	}
	// set profile
	for (j=1; j<=b->length; j++) { int a = (int)(b->seq[0]->residue[j]-'A')+1;
		if (a<1 || a>=NACID) a = 0;
		b->prf[a][j] = 1;
	}
}
...
get_user_aln(key,user,maxs,mins,in)	// NB nor profile setup yet
FILE	*user;
int 	key,maxs, mins, in;
{
	Blocks	*b;
	...
	b = block[nblocks] = (Blocks*)malloc(sizeof(Blocks)); TEST(b)
	nblocks++;
	b->seq = (Seqs**)malloc(sizeof(Seqs*)*members); TEST(b->seq)
	b->members = members;
	b->grouped = 0;
	b->key = key;
	if (key) nkey++;
	...
	{	Seqs	*s;
		...
		if (!n && strstr(line," = ")) {
			...
			if (strlen(c) > TITLE) c[TITLE] = 0;
			strcpy(s->title,c);
			c = (char*)strstr(line," = "); *c = 0;
			n = 1;
		}
		if (!n && strstr(line," : ")) {
			c = (char*)(strstr(line," : ")+3);
		...
		s->code[5] = 0;
		if (strlen(line) > CODE-5) line[CODE-5] = 0;
		strcat(s->code,line);
	...
	{	Seqs	*s = b->seq[i];
		...
		// set profile
		for (j=1; j<=b->length; j++) { int a = (int)(s->residue[j]-'A')+1;
			if (a<1 || a>=NACID) a = 0;
			if (b->prf[a][j] < 255) b->prf[a][j]++;
		}
	}
	return next_aln;
}
...
{	int	i, j, k;
	...
	{ 	Blocks	*b = block[i];
		...
		for ( j=0; j<b_memb; j++ )
		{ Seqs	*s = b->seq[j];
		  int	l = strlen(s->tree)-1;
			if (s->code[0]=='A') {
				s->tree[l] = '|';
				for (k=0; k<l; k++) if(s->tree[k]=='-') s->tree[k] = '=';
				if (b->seq[j+1]->code[0]=='U') s->tree[k] = '#';
			}
		...
		for (k=1; k <= b->length; k++) { int ngap = 0;
			for ( j = 0; j < b_memb; j++ )
			{	Seqs	*s = b->seq[j];
				char	res = s->residue[k];
				if (res<'A' || res>'Z') ngap++;
			}
			if (ngap==b_memb) continue;
			for ( j = 0; j < b_memb; j++ )
			{	Seqs	*s = b->seq[j];
				char	res = s->residue[k];
				if (res<'A' || res>'Z') res = GAP;
				fprintf(aln,"%c",res); 
			}
			fprintf(aln,"\n");
		}
	...
	{	Blocks	*bi = block[i];
		...
		{	Blocks	*bj = block[j];
			...
			if ( npairs >= pair_alloc ) {
				...
				pair = (Pairs*)realloc(pair,sizeof(Pairs)*pair_alloc); TEST(pair)
				*scores = (int*)realloc(*scores,sizeof(int)*pair_alloc); TEST(*scores)
			}
		}
	}
	if ( !npairs ) {
	...
	int	*place;
{
	...
	{	int	a = (pair+place[i])->block_a,	
		...
		{	int	old_alloc = list_alloc;
			...
			list = (Lists*) realloc(list,sizeof(Lists)*list_alloc); TEST(list)
		}
	...
	Groups	*cluster;
{
	...
	aln_len = max_leng*2*max_lines+1;
	alignment = (char*)malloc(sizeof(char)*aln_len); TEST(alignment)
	for(i=0;i<aln_len;i++) alignment[i] = ' ';
	minco = MAX;
	maxco = MIN;
	enter( aln+1, max_leng/2, 0, max_leng*2, 1 );
	max_leng += max_leng;
	l = 0;
	for ( i = 0; i < grp_memb; i++ ) 
	{	Blocks	*b = cluster->blk[i];
		int	new_len = maxco-minco+1,
		...
		// reallocate profile (if need) and rezero
		for ( j = 0; j < NACID; j++) {
			if (relocate) { b->prf[j] = (unsigned char*)realloc(b->prf[j],size); TEST(b->prf[j]) }
			for (k=0; k<=new_len; k++) b->prf[j][k] = 0;
		}
		// reallocate sequence (if need) and copy
		for ( j = 0; j < b->members; j++)
		{	Seqs	*s = b->seq[j];
			int	n = 0;
			if ( relocate ) { 
				s->residue = (char*)realloc(s->residue,size); TEST(s->residue)
			}
			for ( k = minco; k <= maxco; k++)
			{ int	a = (int)(alignment[k+l]-'A')+1;
				if (a<1 || a>=NACID) a = 0;
				s->residue[++n] = alignment[k+l];
				if ( b->prf[a][n] < 255 ) {
					b->prf[a][n]++;	// profile row for acid a
				}
			}
			l += max_leng;
		}
...
int	col, row, leng, pos;
{
	...
	if (a) {	int	i;
		for ( i = 0; i < memb_a; i++)
		{	Seqs	*s = blk_a->seq[i];
			alignment[cell] = s->residue[a];
			if (a<1 || a>blk_a->length) alignment[cell] = ' '; else alignment[cell] = s->residue[a];
	...
	if (b) {	int	i;
		for ( i = 0; i < memb_b; i++)
		{	Seqs	*s = blk_b->seq[i];
			if (b<1 || b>blk_b->length) alignment[cell] = ' '; else alignment[cell] = s->residue[b];
			if ( col > colmax[row] ) colmax[row] = col;
			cell += leng;
			row++;
		}
...
update_blocks ()
{
	...
	{	Groups	*g = group+i;
		...
		{	Blocks	*b = g->blk[j];
			int	b_memb = b->members,
				new_size = ((core->members)+b_memb)*(sizeof(Seqs*));
			core->seq = (Seqs**)realloc(core->seq,new_size); TEST(core->seq)
			printf("%d\n",b->score_to_last);
			link = 0;
			top = 999;
		...
		// update core profile
		for (j=1; j<g_memb; j++ ) // add in profiles from blocks following the core (0)
		{ Blocks *b = g->blk[j];
			for (k=0; k<NACID; k++) {
				for (l=1; l<=b->length; l++) {
					if (core->prf[k][l]+b->prf[k][l] > 255) {
						core->prf[k][l] = 255;
					} else {
						core->prf[k][l] += b->prf[k][l];
					}
				}
			} 
		}
	...
	Alns	*aln;
{
	...
	int	*posa, *posb,
		...
		mati, matj, 
		i, j;
	int	toprows[MAXWINLEN];
	float	maxrows[MAXWINLEN],
		maxscore = -9999.9, 
		mat[2][MAXWINLEN],
		mul[2][MAXWINLEN];
	char	na, nb;
	for ( i = 0; i < window; i++ ) {
		...
		mat[0][i] = -9999.9;
		mat[1][i] = -9999.9;
		pat[i][lenb] = 0; // 0 = exit in traceback()
		maxrows[i] = -9999.9;
		toprows[i] = lenb;
	}
	...
	{ int	wmid = lena-(lenb-j), topcol = 0;
	  float	maxcol = -9999.9;
		...
		{ int	wi = i+wdisp, rat = enter+wi;
		  float	maxrow, diag, row, col;
		  int	toprow;
			pat[wi][j] = 0;
			if (block_aln) {
				mat[now][wi] = prof( blka, blkb, i, j );
			} else {
				na = UPPER(seqa->residue[i])-'A';
				mat[now][wi] = (float)matrix[na][nb];
			}
			...
			toprow = toprows[rat];
			diag = mat[last][wi];
			row = maxrow-(float)gap_pen; 
			col = maxcol-(float)gap_pen; 
			if (mul_wt) {
				mul[now][wi] = ((1+mat[now][wi])*(1+mul[last][wi]))/(float)damp;
				diag += (mul[now][wi]*(float)mul_wt)/100.0; 
			}
			if ( diag >= row && diag >= col ) {
				...
				if ( col > row ) {
					mat[now][wi] += col;
					mul[now][wi] = 0;
					if (trace_back) pat[wi][j] = topcol-i+1;
				} else {
					mat[now][wi] += row;
					mul[now][wi] = 0;
					if (trace_back) pat[wi][j] = -(toprow-j+1);
				}
			}
			if (diag > maxcol) {
				maxcol = diag;
	...
	if ( !trace_back ) return (int)maxscore;
	posa = aln->position_a;
	posb = aln->position_b;
	aln->length = 0;
	...
	if ( *posa == lena ) {
		...
		if ( aln->length > MAXALNLEN ) printf("*NB* aln_length exceeded\n");
		return (int)maxscore;
	}
	if ( *posb == lenb ) {
	...
	if ( aln->length > MAXALNLEN ) printf("*NB* aln_length exceeded\n");
	if ( aln->length < lena) { Pi(aln->length) Pi(lena) Pi(lenb) NL exit(1); }
	if ( aln->length < lenb) { Pi(aln->length) Pi(lenb) Pi(lena) NL exit(1); }
	return maxscore;
}

trace ( posa, posb, mati, matj, pati, patj, lasti, lastj, n )
int	*posa,*posb,mati, matj, pati, patj, lasti, lastj,*n;
{
	int	pij = pat[pati][patj], i, j;
	for ( i=lasti+1; i < mati; i++) { *(++posa) = i; *(++posb) = 0; (*n)++; } 
	for ( j=lastj+1; j < matj; j++) { *(++posa) = 0; *(++posb) = j; (*n)++; }
...
float prof (a, b, pos_a, pos_b)
Blocks	*a, *b;
int	pos_a, pos_b;
{
register	i, j, sum = 0;
int	amemb = a->members, bmemb = b->members;
float	score;
	if (amemb*bmemb < 30) { // sum of pairs
		for ( i = 0; i < amemb; i++ ) 
		...
		score = (float)sum/(float)(amemb*bmemb);
	} else {	// use profile/profile
		sum = 0;
		for (i=1; i<NACID; i++) {
			for (j=1; j<NACID; j++) { int s;
				sum += matrix[i-1][j-1]*(int)a->prf[i][pos_a]*(int)b->prf[i][pos_b];
			}
		}
		score = (float)sum/(float)(amemb*bmemb);
	}
	if (mul_wt==0 && damp > 0) { float boost; // up score to a pre-alignment
		boost = 1.0 + 0.1*(float)damp;
		if (a->seq[0]->code[0]=='A' && b->seq[0]->code[0]=='U') score *= boost;
		if (b->seq[0]->code[0]=='A' && a->seq[0]->code[0]=='U') score *= boost;
	}
	return score;
}
