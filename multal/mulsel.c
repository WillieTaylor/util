	...
	{ int	*pep_order;
	  int	i;
		...
		{ int	aspan, bspan, minp, hold, diag,
			io = read_line(run_data,line);
			sscanf(line,"%d%d%d%d%d", &aspan,&bspan,&minp,&hold,&diag);
		...
		{ Seqs	*s = b->seq[j];
		  ...
		  float resol, bvalue;
			...
			resol = log(resol*resol+1.0);
                        ...
                        if (strstr(s->code,"PDB:")) resol -= 80.0;
                        if (strstr(s->code,"PDB;")) resol -= 80.0;
                        ...
                        if (strstr(s->code,"RFAM")) resol -= 50.0;
...
*/
	...
	delayed = 0; ///////////////////////////////// delay option turned off
...
get_param ()
{
int	listlim, old_mat_wt = mat_wt,
	io = read_line(run_data,line);
	if ( io < 10 ) return 0;
	sscanf(line,"%d%d%d%d%d%d%d%d%d%d", &mul_wt,&damp,&mat_wt,
